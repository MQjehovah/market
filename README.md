...
**模型线上网关（不下载 zip）**：客户端只连 `marketplace_mcp`（或直调 `/api/runtime/*` / `/api/mcp-gateway/relay/{name}/*`），按授权在线调能力。
...
| MCP HTTP 网关 | `/api/mcp-gateway/{name}` | 管理员登记的平台连接器（服务级） |
| 能力 MCP 网关 | `/api/mcp-gateway/relay/{name}/sse`（可 `name@version`；`/cap/{name}/…` 过渡别名） | 商品 MCP 代理（桌面 `gateway-sse`）；限流/熔断/审计 |
...
**Consume**：模型线上网关 `marketplace_mcp` → `/api/runtime/*`、A2A、`/api/mcp-gateway/relay/{name}/*`；员工装机 `host-sync` / `cap install`
